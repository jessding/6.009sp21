...
# my test for acted_together
def test_acted_together_mine():
    actor1 = 9210
    actor2 = 16935
    assert lab.acted_together(db_tiny, actor1, actor2) == False
...
# my tiny bacon number tests
def test_tiny_bacon_number_0():
    n = 0
    expected = {4724}
    assert lab.actors_with_bacon_number(db_tiny, n) == expected
...
# my actor to actor test
def test_actor_to_actor_path_mine():
    id_1 = 29817
    id_2 = 18975
    result = lab.actor_to_actor_path(db_large, id_1, id_2)
    assert result == [29817, 89609, 148063, 1053717, 51944, 18975]
...
def check_valid_path(f, p, s, e, l):
    lp = len(p)-1 if p is not None else None
